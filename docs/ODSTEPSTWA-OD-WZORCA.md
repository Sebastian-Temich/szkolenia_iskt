...
| 6.8 | Sekcja kontaktu zawiera działający wizualnie formularz, który niczego nie wysyła (podziękowanie mimo braku wysyłki) | Sekcja z danymi kontaktowymi i **działającym formularzem** (blok `iskt/formularz-zgloszeniowy`, zadanie 9) | Podziękowanie pokazujemy dopiero po przyjęciu wiadomości przez mechanizm wysyłkowy; `wp_mail()` zwracające `false` daje komunikat błędu. Zachowanie prototypu naprawiamy, nie odtwarzamy (§5). Zakres: `docs/FORMULARZ-ZGLOSZENIOWY.md` |
...
| 8.8 | Przycisk zgłoszenia otwiera formularz na stronie głównej, bez informacji o tym, którego szkolenia dotyczy | Odnośnik do strony `/zgloszenie/` z kontekstem w adresie; formularz sam uzupełnia szkolenie, a odnośnik przy terminie także datę | §5 wymaga automatycznego uzupełnienia szkolenia i terminu. Osobny adres działa też przy otwarciu w nowej karcie i po odświeżeniu (§11 pkt 6), czego stan komponentu nie daje |
